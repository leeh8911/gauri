...
OpenGLTexture2D::OpenGLTexture2D(const std::string &path) : m_Path(path)
{
    ...
    {
        GR_PROFILE_SCOPE("stbi_load - OpenGLTexture2D::OpenGLTexture2D(const std::string &path)")
...
void OpenGLTexture2D::SetData(void *data, uint32_t size)
{
    ...
    GR_CORE_ASSERT(size == m_Width * m_Height * bytePerPixel, "Data must be entire texture!")
